...
test("multiple selections can be set by changing in place the selection array when multiple=true", function() {
  ...
  select.set('optionLabelPath', 'content.firstName');
  select.set('multiple', true);
  select.set('selection', Ember.A(selection));
...
test("multiple selections can be set indirectly via bindings and in-place when multiple=true (issue #1058)", function() {
  var SelectTest = window.SelectTest = Ember.Object.create();
  ...
  Ember.run(function() {
    select = Ember.Select.extend({
      contentBinding: 'SelectTest.controller.content',
      selectionBinding: 'SelectTest.controller.selection',
      multiple: true,
      optionLabelPath: 'content.firstName'
    }).create();

    SelectTest.set('controller', Ember.Object.create({
  ...
  Ember.run(function() {
    SelectTest.setPath('controller.content', Ember.A([david, cyril]));
    SelectTest.setPath('controller.selection', Ember.A([cyril]));
  });
