...
function selectedOptions() {
  var rv = [];
  for( var i=0; i<select.getPath('content.length'); ++i ) {
...
test("Ember.SelectedOption knows when it is selected when multiple=false", function() {
  ...
  deepEqual(selectedOptions(), [false, false, false, true], "After changing it, seleciton should be correct");
});
...
test("Ember.SelectedOption knows when it is selected when multiple=true", function() {
  ...
  deepEqual(selectedOptions(), [false, true, true, false], "After changing it, seleciton should be correct");
});
